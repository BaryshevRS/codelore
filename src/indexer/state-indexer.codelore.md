...
Строит индекс документации из состояний, хранящихся в [`DocStateStorage`](../storage/doc-state-storage.codelore.md#docstatestorage), для поиска секций по идентификатору, файлу и сущности.

## Что делает

- Загружает состояния всех документов через [`DocStateStorage`](../storage/doc-state-storage.codelore.md#docstatestorage) и преобразует их в индексные секции с обратными картами (`fileToSections`, `entityToSections`).
...
- [`DocStateStorage`](../storage/doc-state-storage.codelore.md#docstatestorage) (`src/storage/doc-state-storage.ts`) — чтение списка и загрузка состояний секций.
- `CodeloreError` ([`src/errors.ts`](../errors.codelore.md)) — формирование ошибки при дублирующемся идентификаторе секции.
...
- `CodeloreError` ([`src/errors.ts`](../errors.codelore.md)) — формат ошибки дублирования секций, используется в `addSectionToIndex`.
- [`DocStateStorage`](../storage/doc-state-storage.codelore.md#docstatestorage) (`src/storage/doc-state-storage.ts`) — загрузка состояний документов через `loadDocState`.
