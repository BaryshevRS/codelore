...
- Пустой `targets` возвращает пустое множество: цикл группировки не создаёт ни одной записи в `byFile`, и `Promise.allSettled` обрабатывает пустой набор.
- Секции файла, для которого [`collectDependencyDocs`](#collectdependencydocs) вернул пустой массив, никогда не попадают в результат — `continue` пропускает файл до вызова LLM.
- Секции, у которых все целевые блоки имеют пустой текст, тоже не попадают в результат: блоки с `body.trim() === ""` исключаются фильтром `block.body.trim() !== ""`, и при отсутствии оставшихся блоков `continue` завершает обработку.
- В результат включается секция целиком, только если ни один из проверенных блоков не оказался в множестве `contradicted` из [`parseVerifyResponse`](doc-verifier.codelore.md#parseverifyresponse); противоречие в одном блоке исключает всю секцию, без детализации, какие блоки нарушены.
- Функция не мутирует аргументы и не выполняет повторных попыток: ошибка `provider.complete`, [`parseVerifyResponse`](doc-verifier.codelore.md#parseverifyresponse) или `readFile` пробрасывается вызывающему, а не глотается через `allSettled` (он гасит только ошибки внутри самих обработчиков файлов).
- Результат не зависит от порядка завершения файловых задач: `verifiedClean` наполняется решениями каждого файла независимо, хотя порядок добавления при `Promise.allSettled` не детерминирован.
...
Единственный вызывающий — метод `CodeloreService.gateDepDocsCascade` ([`src/service/codelore-service.ts`](../service/codelore-service.codelore.md#codelore-servicets), сигнатура `gateDepDocsCascade(scope: RefreshStaleDocsInput["scope"], runtime: GenerateDocsRuntime)`) — это финальный гейт каскада после переписывания зависимостей: по комментарию в коде он «fires after the last write, when a central file's rewrite has left depDocs drift on each of its dependents». Метод передаёт цели — пары `{ sectionId, blockIds }` — и использует возвращённое множество как список секций, которые верификатор подтвердил по текущей документации зависимостей; контракт зафиксирован в doc-комментарии функции: «only a section confirmed clean by the verifier is safe to clear with a fingerprint refresh». Секции вне множества (не подтверждённые, непроверенные или потерянные из-за сбоя) `gateDepDocsCascade` оставляет помеченными и пускает в обычную регенерацию.

Что происходит на стороне функции:
- Цели группируются по файлу-владельцу (`owns[0]` → `path`), и каждый файл обрабатывается отдельной задачей; задачи файлов параллелятся пулом `pLimit(service.config.llm.concurrency)` — тем же лимитом, что и остальная генерация.
- На каждый файл выполняется один запрос через переданный в вызове `args.provider` (отдельного «верифицирующего» провайдера у гейта нет).
- Сбой провайдера — не ошибка всего прогона: `Promise.allSettled` гасит исключение задачи, секции этого файла просто не попадают в результат. Комментарий в коде описывает это как «absence of evidence»: секция остаётся помеченной и уходит в регенерацию, а не в исключение. Возвращённое множество не различает «проверено и чисто» и «не проверено из-за сбоя» — вызывающий видит только, надо ли ещё раз генерировать секцию.

## Чего не делает

- У каждого файла ровно одна попытка: `await provider.complete(buildVerifyRequest(…))` не обёрнуто в цикл ретраев. Исключение из `readFile`, `provider.complete` или [`parseVerifyResponse`](doc-verifier.codelore.md#parseverifyresponse) гасится `Promise.allSettled` — секции такого файла молча не попадают в `verifiedClean`, и вызывающий не узнаёт о самом сбое.
- Верификатор видит усечённую документацию зависимостей: [`collectDependencyDocs`](#collectdependencydocs) режет блоки по `DEP_DOC_FILE_CAP` (2500) и останавливает сбор по `DEP_DOCS_TOTAL_CAP` (15000) с `break`. Утверждение, которое опирается на отрезанный кусок, провайдер не увидит и не сможет зафиксировать ни подтверждённым, ни противоречащим — секция может пройти как чистая лишь потому, что противоречащий текст не попал в запрос.

## Как менять и что проверять

- Число LLM-обращений жёстко равно числу файлов-владельцев: каждая запись `byFile` образует ровно одну задачу через `limit(async () => {` внутри `[...byFile].map(...)`, и в ней один `await provider.complete(...)` — добавить повторную проверку или чанкинг значит менять этот один обработчик.
- Проверяются только запрошенные блоки с непустым текстом: фильтр `wanted.has(block.id as BlockId) && block.body.trim() !== ""` в `section.blocks.filter(...)` и ранний `if (blocks.length === 0) { continue; }` — блок, не названный в `targets` или с пустым телом, никогда не попадёт в `VerifySectionInput` и не повлияет на результат.
- Цель с отсутствующим владельцем в индексе молча исключается ещё до группировки: путь получается через `index.code.entities[ownerId]?.path`, и `if (!file) { continue; }` просто пропускает такую секцию — вызывающий не получит о ней ни подтверждения, ни ошибки, только «не проверена».
