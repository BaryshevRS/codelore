...
export interface GenerateDocsForScopeInput {
  ...
  force?: boolean;
  /**
   * Narrow generation to these blocks. Each section still keeps only the ones its
   * own analysis allows, so a block that does not apply to an entity is dropped
   * rather than forced. Omitted = every block the section allows.
   */
  blocks?: BlockId[];
  /**
   * Run the LLM fact-check pass over freshly written blocks. Default true. Turning
   * it off trades the contradiction check for roughly a third of the run's prompt
   * volume; the deterministic reference validation always runs either way.
   */
  verifyFacts?: boolean;
}
...
export class CodeloreService {
  ...
  async generateDocsForScope(
    ...
    const targets = input.blocks
      ? narrowTargetBlocks(targetBlocksBySection, sourceIds, input.blocks)
      : targetBlocksBySection;
    const result = await this.runPipelineForSections(
      sourceIds,
      input.intent,
      runtime,
      targets,
      input.verifyFacts ?? true
    );
