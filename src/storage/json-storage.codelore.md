...
1. [`CodeloreService`](../service/codelore-service.codelore.md#codeloreservice) вызывает `saveProjectIndex` после анализа кода для сохранения индекса проекта.
2. [`CodeloreService`](../service/codelore-service.codelore.md#codeloreservice) вызывает `loadProjectIndex` при старте для загрузки индекса.
3. [`CodeloreService`](../service/codelore-service.codelore.md#codeloreservice) вызывает `saveChangeAnalysis` для сохранения результатов анализа изменений и `loadChangeAnalysis` для их загрузки по идентификатору.
