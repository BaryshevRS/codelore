...
Вызывается из buildCodeIndex ([src/indexer/code-indexer.ts](../indexer/code-indexer.codelore.md)) при построении CodeIndex. Возвращаемый Map используется в populateDocumentationMetadata для наполнения метаданных каждой сущности.
...
- Возвращает true при совпадении имени вызываемой функции с DEFAULT_SIDE_EFFECT_CALLS — проверка в `if (DEFAULT_SIDE_EFFECT_CALLS.has(expressionText) || DEFAULT_SIDE_EFFECT_CALLS.has(propertyName))`.
