...
* Не выполняет бизнес-логику команд — делегирует [`CodeloreService`](/docs/src/service/codelore-service#codeloreservice) или [`startStdioServer`](/docs/src/server/start-stdio-server#startstdioserver).
* Не управляет MCP-сервером после запуска — [`startStdioServer`](/docs/src/server/start-stdio-server#startstdioserver) управляет своим жизненным циклом.
...
* Любое исключение перехватывается и выводится в `stderr` как JSON с полями `ok: false` и `error` через [`toCodeloreErrorPayload`](/docs/src/errors#tocodeloreerrorpayload).
* Результат (если не `undefined`) выводится в `stdout` как JSON.

## От чего зависит

runCli использует три внешних модуля: [`toCodeloreErrorPayload`](/docs/src/errors#tocodeloreerrorpayload) из `src/errors.ts` — сериализует перехваченные исключения в JSON для вывода в `stderr`; [`startStdioServer`](/docs/src/server/start-stdio-server#startstdioserver) из `src/server/start-stdio-server.ts` — запускает MCP-сервер для команды `mcp`; [`CodeloreService`](/docs/src/service/codelore-service#codeloreservice) из `src/service/codelore-service.ts` — предоставляет методы для всех остальных команд (делегируется через `runCommand`).
...
3. Если команда `mcp` — вызывает [`startStdioServer`](/docs/src/server/start-stdio-server#startstdioserver) и после его успешного завершения возвращает 0.
4. Для остальных команд создаётся [`CodeloreService`](/docs/src/service/codelore-service#codeloreservice), вызывается `runCommand`, который проверяет аргументы через `assertExpectedArgs`, парсит специфичные аргументы (через `parseScope`, `parseAnalyzeChangeInput`, `parseRefreshStaleDocsInput`) и вызывает соответствующий метод сервиса.
...
7. Любое исключение перехватывается, сериализуется через [`toCodeloreErrorPayload`](/docs/src/errors#tocodeloreerrorpayload) и выводится в `stderr` как JSON с полями `{ ok: false, error: ... }`, возвращается 1.
