...
Вызывается из `isAcceptedLiteralNode` в [`src/context/ast-hit-filter.ts`](/docs/src/context/ast-hit-filter) и `isAcceptableLiteral` в [`src/context/literal-extractor.ts`](/docs/src/context/literal-extractor). Обе функции проверяют, что строковый литерал не находится внутри `import`/`export`, прежде чем считать его валидным литералом для дальнейшей обработки.
...
Вызывается из `isAcceptedLiteralNode` в [`src/context/ast-hit-filter.ts`](/docs/src/context/ast-hit-filter) и `isAcceptableLiteral` в [`src/context/literal-extractor.ts`](/docs/src/context/literal-extractor). Обе функции проверяют, что строковый литерал не находится внутри JSDoc, прежде чем считать его валидным литералом для дальнейшей обработки.
...
Вызывается из `isAcceptedLiteralNode` в [`src/context/ast-hit-filter.ts`](/docs/src/context/ast-hit-filter) и `isAcceptableLiteral` в [`src/context/literal-extractor.ts`](/docs/src/context/literal-extractor). Обе функции проверяют, что строковый литерал не находится внутри type-аннотации, прежде чем считать его валидным литералом для дальнейшей обработки.
...
Вызывается из `isAcceptedLiteralNode` в [`src/context/ast-hit-filter.ts`](/docs/src/context/ast-hit-filter) и `isAcceptableLiteral` в [`src/context/literal-extractor.ts`](/docs/src/context/literal-extractor). Обе функции проверяют, что строковый литерал не имеет контекстного типа-литерала, прежде чем считать его валидным литералом для дальнейшей обработки.
...
Вызывается из [`resolveEnclosing`](/docs/src/context/enclosing-resolver#resolveenclosing) в `src/context/enclosing-resolver.ts` и [`extractLiterals`](/docs/src/context/literal-extractor#extractliterals) в `src/context/literal-extractor.ts`. В [`resolveEnclosing`](/docs/src/context/enclosing-resolver#resolveenclosing) используется для заполнения поля `syntaxRole` в `RuntimeHit`. В [`extractLiterals`](/docs/src/context/literal-extractor#extractliterals) — для заполнения поля `syntaxRole` в `ExtractedLiteral`.
