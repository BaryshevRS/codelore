...
* [`planWriterChunks`](/docs/src/llm/file-chunks#planwriterchunks) (src/llm/file-chunks.ts): обрезает комментарии из каждого файла источника перед сборкой в чанки. Если весь файл влезает в лимит символов, применяет `stripComments` ко всему файлу сразу.
* `buildVerifySections` ([src/llm/file-pipeline.ts](/docs/src/llm/file-pipeline)): обрабатывает срез исходного кода сущности (от `startOffset` до `endOffset`) перед передачей в верификатор.
* `verifyBlocksAgainstDeps` ([src/llm/file-pipeline.ts](/docs/src/llm/file-pipeline)): аналогично `buildVerifySections`, обрабатывает срезы для верификации блоков документации.
* [`collectTypeContext`](/docs/src/llm/type-context#collecttypecontext) (src/llm/type-context.ts): очищает извлечённые объявления типов от комментариев для отправки в контекст LLM.
...
* Изменение набора `DIVISION_CONTEXT_TOKENS` — инвариант, закреплённый в строгом перечислении токенов (`ts.SyntaxKind.Identifier` … `ts.SyntaxKind.MinusMinusToken`), проверять в [`planWriterChunks`](/docs/src/llm/file-chunks#planwriterchunks), [`collectTypeContext`](/docs/src/llm/type-context#collecttypecontext) и обоих вызовах из `file-pipeline.ts`.
