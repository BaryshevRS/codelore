...
Импортирует тип `ChatCompletionInput` из [`src/llm/provider.ts`](/docs/src/llm/provider) и тип `DependencyDoc` из [`src/llm/file-writer.ts`](/docs/src/llm/file-writer). Не использует никакие другие внешние модули во время выполнения — все остальные импорты файла относятся к другим функциям.

## Кто и как использует

Вызывается из [`verifyChunkFacts`](/docs/src/llm/file-pipeline#verifychunkfacts) (статическая, `src/llm/file-pipeline.ts`), [`verifyBlocksAgainstDeps`](/docs/src/llm/file-pipeline#verifyblocksagainstdeps) (статическая, там же) и `buildRepairRequest` (там же, с `runtimeEvidence` совпадающей строки инструкции "Return only valid JSON. Do not include Markdown fences."). Построенный `ChatCompletionInput` передаётся в `provider.complete`. Промпт собирается для каждого файла отдельно в цикле по `fileTargets` внутри [`verifyBlocksAgainstDeps`](/docs/src/llm/file-pipeline#verifyblocksagainstdeps).
...
* Выбрасывает [`CodeloreError`](/docs/src/errors#codeloreerror) с кодом `INVALID_LLM_RESPONSE`, если `contradictions` не является массивом.
...
* Если `contradictions` — не массив, выбрасывает [`CodeloreError`](/docs/src/errors#codeloreerror) с кодом `INVALID_LLM_RESPONSE`.

### От чего зависит

* `src/errors.ts` — [`CodeloreError`](/docs/src/errors#codeloreerror) для выбрасывания исключения при невалидном формате поля `contradictions` (не массив).
* `src/llm/json.ts` — [`parseJsonObject`](/docs/src/llm/json#parsejsonobject) для первичного парсинга JSON из ответа LLM.

### Кто и как использует

Вызывается из [`verifyBlocksAgainstDeps`](/docs/src/llm/file-pipeline#verifyblocksagainstdeps) и [`verifyChunkFacts`](/docs/src/llm/file-pipeline#verifychunkfacts) (оба в `src/llm/file-pipeline.ts`). В [`verifyBlocksAgainstDeps`](/docs/src/llm/file-pipeline#verifyblocksagainstdeps) результат `parseVerifyResponse` преобразуется в `Set` sectionId, прошедших проверку. В [`verifyChunkFacts`](/docs/src/llm/file-pipeline#verifychunkfacts) возвращённые `ContradictedClaim` затем отображаются в `DocViolation` для ремонта.
