...
* [`stripComments`](/docs/src/llm/strip-comments#stripcomments) (`src/llm/strip-comments.ts`) — очищает исходный код от комментариев для уменьшения объёма, передаваемого в LLM.

## Кто и как использует

Вызывается из [`generateFileGroup`](/docs/src/llm/file-pipeline#generatefilegroup) (`src/llm/file-pipeline.ts`) после того, как та собрала исходные файлы и секции документации. [`generateFileGroup`](/docs/src/llm/file-pipeline#generatefilegroup) передаёт в `planWriterChunks` массив `sources` (содержимое файлов, прочитанное через `readFile`), массив `sections` (сформированных через `fileWriteSection`), `code` из `index.code`, `budget` из `provider.writerBudget` и `overheadChars`. Результат — массив `WriterChunk[]` — [`generateFileGroup`](/docs/src/llm/file-pipeline#generatefilegroup) передаёт в `generateChunk` для каждого чанка, параллельно через `pLimit`.
