...
Вызывается из [`CodeloreService.validateDocs`](/docs/src/service/codelore-service#validatedocs). Метод перестраивает индексы, затем для каждой секции вызывает `validateOwnedEntities`, передавая секцию и словарь code-сущностей. Функция итерирует по `section.owns` и при отсутствии сущности в `codeEntities` формирует `DocValidationIssue` с кодом `stale_owned_entity` и уровнем `error`.
...
* Не проверяет блоки, чей идентификатор не распознан [`isKnownBlockId`](/docs/src/service/helpers#isknownblockid).
...
* [`isKnownBlockId`](/docs/src/service/helpers#isknownblockid) (`src/service/helpers.ts`) — проверяет, распознан ли идентификатор блока.
* [`staleValidationIssue`](#stalevalidationissue) (`src/service/doc-validation.ts`) — формирует issue для одного блока.

### Кто и как использует

Функция проверяет наличие owned-сущностей; при отсутствии возвращает пустой массив. Для каждого блока секции с известным `block.id` (проверка через [`isKnownBlockId`](/docs/src/service/helpers#isknownblockid)) делегирует проверку устаревания в [`staleValidationIssue`](#stalevalidationissue).
...
* Вычисляет текущий отпечаток блока через [`computeBlockFingerprint`](/docs/src/markdown/block-facets#computeblockfingerprint).
* Если блок помечен как удалённый (`staleSince`), возвращает issue с `drift: "tombstoned"`.
* Иначе сравнивает `storedValue` и `currentValue`; при расхождении определяет изменившиеся фасеты через [`diffFacetHashes`](/docs/src/markdown/block-facets#difffacethashes).
...
* [`parseBlockFingerprintValue`](/docs/src/markdown/block-facets#parseblockfingerprintvalue) (`src/markdown/block-facets.ts`) — парсит сохранённую строку отпечатка.

### Кто и как использует

Вызывается из [`detectStaleBlocks`](#detectstaleblocks) для каждого блока секции. Функция сначала пытается вычислить текущий отпечаток через [`computeBlockFingerprint`](/docs/src/markdown/block-facets#computeblockfingerprint). Если он не вычислен, возвращает `undefined`. Иначе, если блок помечен как удалённый (`staleSince`), возвращает issue с типом `tombstoned`. В противном случае сравнивает сохранённый и текущий отпечатки; при расхождении вызывает [`diffFacetHashes`](/docs/src/markdown/block-facets#difffacethashes) для определения изменившихся фасетов и формирует issue.
