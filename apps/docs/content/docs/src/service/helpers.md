...
1. [`CodeloreService.analyzeChange`](/docs/src/service/codelore-service#analyzechange) — для объединения `input.changedFiles` с путями из `parseUnifiedDiff(diff)`.
...
3. [`docStateSectionForPlannedEntity`](/docs/src/service/prepare-docs#docstatesectionforplannedentity) — для сортировки `entity.directDeps` и `entity.directUsages`.
4. [`normalizePrepareScope`](/docs/src/service/prepare-docs#normalizepreparescope) — для дедупликации и сортировки `paths`, `files` и `entityIds`.
5. [`skippedPreparedDocSection`](/docs/src/service/prepare-docs#skippedprepareddocsection) — для сортировки `docPaths`.
6. [`mergeGeneratedBlocksIntoSection`](/docs/src/service/section-rewrite#mergegeneratedblocksintosection) — для сортировки `owner.directDeps` и `owner.directUsages`.
...
Вызывается в [`CodeloreService.getSectionContext`](/docs/src/service/codelore-service#getsectioncontext) для дедупликации и сортировки массива `directUsages`. Для каждой сущности из `owns` собираются идентификаторы использований, затем по ним извлекаются объекты `CodeEntity` из `index.code.entities`, и результат передаётся в `uniqueEntities`.
...
Вызывается в [`CodeloreService.rewriteSections`](/docs/src/service/codelore-service#rewritesections) для проверки дубликатов `sectionId` во входном массиве. Если `firstDuplicate` возвращает строку, выбрасывается ошибка с кодом `DUPLICATE_SECTION`.
...
* Функция возвращает `undefined` при отсутствии дубликатов — вызывающий ([`CodeloreService.rewriteSections`](/docs/src/service/codelore-service#rewritesections)) проверяет результат через `if (duplicateSectionId)`, что корректно обрабатывает `undefined`. Тест: не указан.
...
1. [`CodeloreService.getSectionContext`](/docs/src/service/codelore-service#getsectioncontext) — для преобразования сущностей `owns`, `depends` и `directUsages` в краткое представление.
2. [`buildRewriteContextBundle`](/docs/src/service/stale-detection#buildrewritecontextbundle) — для преобразования сущностей `owns` в контекст для перезаписи.
...
Вызывается в [`CodeloreService.getSectionContext`](/docs/src/service/codelore-service#getsectioncontext) для преобразования полной секции документации в краткое представление, которое включается в возвращаемый `SectionContext`.
...
* Результат зависит только от значения value и константы BLOCK\_IDS.
...
Вызывается в [`detectStaleBlocks`](/docs/src/service/doc-validation#detectstaleblocks) для фильтрации блоков секции перед проверкой на устаревание. Вызывается в [`addTargetBlock`](/docs/src/service/stale-detection#addtargetblock) для проверки валидности идентификатора блока перед добавлением в карту целей. Вызывается в [`collectDepDocStaleBlocks`](/docs/src/service/stale-detection#collectdepdocstaleblocks) для фильтрации блоков при обнаружении устаревших из-за изменений в документации зависимостей. Вызывается в [`deletedOwnedEntityBlocks`](/docs/src/service/stale-detection#deletedownedentityblocks) для фильтрации блоков при обнаружении удалённых сущностей. Вызывается в [`staleBlockInfoFor`](/docs/src/service/stale-detection#staleblockinfofor) для проверки идентификатора блока перед вычислением отпечатка. Вызывается в [`tombstonedBlockInfoFor`](/docs/src/service/stale-detection#tombstonedblockinfofor) для проверки идентификатора блока перед формированием информации о захороненном блоке.
...
Вызывается в [`CodeloreService.runPipelineForSections`](/docs/src/service/codelore-service#runpipelineforsections) при обработке отклонённых промисов и исключений в фазе `apply` — результат добавляется в массив `failed` с полями `error` и `code`.
