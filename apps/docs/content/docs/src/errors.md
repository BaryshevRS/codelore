...
1. Вызывающий код (например, [`src/cli/run-cli.ts`](/docs/src/cli/run-cli), [`src/indexer/state-indexer.ts`](/docs/src/indexer/state-indexer)) создаёт `CodeloreError` с кодом и сообщением при обнаружении ошибочной ситуации.
...
3. Другие компоненты (например, [`src/llm/complete-and-parse.ts`](/docs/src/llm/complete-and-parse), [`src/service/codelore-service.ts`](/docs/src/service/codelore-service)) также выбрасывают `CodeloreError` при невалидных входных данных или внутренних сбоях.
...
Вызывается в [`runCli`](/docs/src/cli/run-cli#runcli) (`symbol:src/cli/run-cli.ts#runCli`) и `createCodeloreServer` (`file:src/server/create-server.ts`) в блоке `catch` для сериализации ошибки в JSON-ответ.
