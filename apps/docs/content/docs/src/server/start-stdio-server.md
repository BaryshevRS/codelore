...
* Создаёт экземпляр MCP-сервера через [`createCodeloreServer`](/docs/src/server/create-server#createcodeloreserver) с переданным `rootDir`.
...
* При каждом вызове создаётся новый экземпляр сервера через [`createCodeloreServer`](/docs/src/server/create-server#createcodeloreserver); состояние между вызовами не разделяется.
...
3. [`createCodeloreServer`](/docs/src/server/create-server#createcodeloreserver) создаёт сервер и регистрирует ресурсы, затем `StdioServerTransport` подключается через `server.connect(transport)`, после чего сервер начинает обработку сообщений через stdin/stdout.
